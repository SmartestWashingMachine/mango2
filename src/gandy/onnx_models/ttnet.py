...
class TTNetONNX(BaseONNXModel):
    def __init__(self, onnx_path, use_cuda):
        ...
        self.tfm = A.Compose(
            [
                A.LongestMaxSize(
                    max_size=256, interpolation=cv2.INTER_NEAREST, always_apply=True
                ),
    ...
    def preprocess(self, inp):
        ...
        inp /= 255.0  # Normalize to get values from 0 to 1.
    ...
    def postprocess(self, outp_data, to_255=False):
        ...
        outp = cv2.resize(
            outp, (original_width, original_height), interpolation=cv2.INTER_NEAREST
        )
