...
class NeighborRedrawApp(ImageRedrawGlobalApp):
    ...
    def process(self, image: PILImage.Image, bboxes, target_texts, text_colors):
        """
        ...
        initial_directions = [
            i for i in range(8)
        ]  # 0 = bottom left, 1 = left, 2 = top left, 3 = up, 4 = top right, etc...
        ...
        for j, s_bbox in enumerate(s_bboxes):
            ...
            for _ in range(4):
                ...
                while len(directions) > 0:
                    picked_direction_val = np.random.choice(
                        directions, p=direction_probs
                    )
                    ...
                    direction_probs = [
                        v + (missing_prob / len(direction_probs))
                        for v in direction_probs
                    ]
                    ...
                    neighbor_box = (
                        ...
                        neighbor_left + neighbor_width,
                        neighbor_top
                        + height,  # Neighbor has same width as the original box.
                    )
        ...
        return super().process(image, bboxes, target_texts, text_colors)
