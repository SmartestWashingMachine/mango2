...
class OnnxWrapper():
    ...
    def __call__(self, x, sr: int):
        ...
        if sr in [8000, 16000]:
            ort_inputs = {'input': x.astype(np.float32), 'state': self._state, 'sr': np.array(sr, dtype='int64')}
...
def read_audio(path: str, sampling_rate: int = 16000) -> np.ndarray:
    """Read audio file via FFmpeg and return mono float32 array.
    
    Matches librosa.load() behavior:
    - Resamples to sampling_rate
    - Downmixes to mono
    - Normalizes to [-1.0, 1.0]
    """
    try:
        # 1. Setup the ffmpeg process to pipe output to stdout
        out, _ = (
            ffmpeg
            .input(path)
    ...
    # PCM float32 is exactly what librosa uses internally
...
def get_speech_timestamps(audio,
    ...
    Parameters
    ----------
    audio: numpy array or list, one dimensional
        One dimensional float array, other types are casted to numpy if possible
...
class VADIterator:
    ...
    def __call__(self, x, return_seconds=False, time_resolution: int = 1):
        ...
        window_size_samples = x.shape[1] if x.ndim == 2 else x.shape[0]
...
def collect_chunks(tss: List[dict],
    ...
    wav: numpy array, one dimensional
        One dimensional float numpy array, containing the audio to clip.
    ...
    numpy array, one dimensional
        One dimensional float numpy array of the concatenated clipped audio
        chunks.
...
def drop_chunks(tss: List[dict],
    ...
    -------
    numpy array, one dimensional
        One dimensional float numpy array of the input audio minus the dropped
        chunks.
